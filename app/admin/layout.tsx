...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-zinc-950 text-zinc-100 flex">
            ...
            <aside className="hidden lg:flex w-72 border-r border-zinc-800/60 bg-gradient-to-b from-zinc-900 to-zinc-950 flex-shrink-0 flex-col">
                <SidebarContent />
            </aside>
            ...
            {sidebarOpen && (
                <div className="fixed inset-0 z-50 lg:hidden">
                    ...
                    />
                    {/* Drawer */}
                    <aside className="absolute left-0 top-0 bottom-0 w-72 bg-gradient-to-b from-zinc-900 to-zinc-950 border-r border-zinc-800/60 flex flex-col shadow-2xl">
                        {/* Close button */}
                        <button
                            onClick={() => setSidebarOpen(false)}
                            className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-lg text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800 transition-colors cursor-pointer z-10"
                        >
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
            ...
            <main className="flex-1 overflow-y-auto min-w-0">
                <header className="sticky top-0 z-10 h-16 border-b border-zinc-800/60 px-4 sm:px-8 flex items-center justify-between bg-zinc-950/80 backdrop-blur-xl">
                    <div className="flex items-center gap-3">
                        {/* Mobile hamburger */}
                        <button
                            onClick={() => setSidebarOpen(true)}
                            className="lg:hidden w-9 h-9 flex items-center justify-center rounded-lg text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800 transition-colors cursor-pointer"
                        >
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
